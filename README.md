...
All four target **Limine** (racknerd BIOS; others UEFI). Headless means no desktop, **not** removal of consoles/emergency recovery. Tailscale enrollment for all four hosts and OpenTofu tailnet policy/MagicDNS management are now **prepared with rollout disabled**; see [the Tailscale procedure](docs/tailscale.md). Only ThinkPad → the other three hosts on TCP 22 and ICMP is planned; all other tailnet peer access, including family devices, is denied. The operator completed read-only policy/DNS imports and a policy-only plan; neither live policy nor DNS has been changed. Gaming applications/drivers beyond ordinary hardware support, reverse proxy, shares and other applications remain deferred. The Samsung display was later observed on `DP-3` with qualifying HDR metadata, but eGPU/GPU topology remains unverified and no NVIDIA driver is selected. The deployed maintenance boot exposed a startup race before the intended docked HDR/lid policy was applied; its fix still needs runtime acceptance. Requested application preferences are deliberately reused without copying private profiles or credentials. Dino's original stateVersion and EFI policy remain unknown, and its boot filesystem needs review.
