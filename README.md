...
**ThinkPad is commissioned and local-only. Its maintenance generation is deployed and booted, but limited runtime review exposed a cold-start output-policy race; the branch contains an unactivated fix and maintenance acceptance remains incomplete. The other hosts remain unready until their transition reviews complete.** Only ready hosts enter `nixosConfigurations`; deploy outputs require readiness plus enabled deployment intent.
...
- `modules/tailscale/`, `tofu/tailscale/`: disabled, review-gated client enrollment/persistence plus separate OpenTofu policy/MagicDNS management. `just tailscale-inventory` shows rollout blockers; `just tailnet` is an explicit operator workflow, never part of rebuild/check.
