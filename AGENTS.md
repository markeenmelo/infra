...
- Native devenv provides unstable developer tools; its Nixpkgs pin must match `flake.lock` and its OpenTofu/provider wrapper must match the checked package. Only the explicitly requested `host:create`, `host:install` and `deploy:run` tasks are registered. Keep them uncached, null-default and disconnected from lifecycle/each other; no automatic formatter, old repo DAG or test-entry gate. Bodies live in `scripts/devenv/`.
