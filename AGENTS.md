...
- `flake.nix` is the production entry point; root `devenv.nix` is the development-only exception and is never imported into production. Every other `.nix` file lives under `modules/` as a top-level flake-parts module, including hardware facts. `modules/` is Nix only: static data goes in `assets/<concern>/`. Native operator HCL lives under `opentofu/tailscale/` and `opentofu/porkbun/`, with separate credential/state custody. The only standalone scripts are `scripts/devenv/install.sh`, `scripts/devenv/deploy.sh` and `scripts/devenv/tailnet.sh`, backing the three explicit development tasks. There is no test suite or general-purpose scripts tree.
...
- `devenv.nix` provides the locked toolbox and references only `fleet:install`, `fleet:deploy` and `tailnet:deploy` under `scripts/devenv/`. These tasks require explicit invocation: no shell-entry dependencies, hooks or automatic operations. Tailnet apply, identity/tag transitions and enrollment each require explicit authorization; a saved plan authorizes none of them. Keep task logic in those three scripts and the development-only package override in Nix. Its nixpkgs pin must match `flake.lock`.
