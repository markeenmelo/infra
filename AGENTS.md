...
- Do not deploy, remotely activate, install, mount, partition, format, destroy data or run generated disko scripts without explicit authorization for that operation in the current task. `--dry-activate` also contacts a target. `devenv tasks run repo:check` is non-destructive.
