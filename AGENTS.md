...
- Never invent hardware, users, keys, endpoints, UUIDs, firmware, GPUs or NAS topology. Unknown facts stay typed nulls or blockers. Never set `fleet.installation.approved`, `confirmed` or a `*Reviewed` flag, use `allowNoPasswordLogin`, or disable rollback just to make something pass.
...
- Track selection belongs in `modules/fleet.nix`. `fleetConfigurations` and `nixosConfigurations` always evaluate every host; deploy nodes require only `deployment.enable`. `fleet.installation.approved` and `fleet.bootstrap.missing` never block a system build — they only make each host's public disko aliases refuse an unapproved or fact-incomplete target.
...
2. Make the smallest coherent change using the existing typed boundaries. Leave facts, installation approval, credentials and pins alone unless they are the task.
