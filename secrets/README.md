...
The shared account policy selects **`shared/marcos-password.yaml`** for `marcos` on all three hosts. ThinkPad and Racknerd declare early delivery; Bastion's candidate account remains locked until its verified dedicated identity/recipient is supplied. This does not change installed passwords.
...
| bastion | None supplied; not a recipient of the shared password yet | Null identity/recipient blockers |
...
The exact shared rule grants only the existing **operator, ThinkPad and Racknerd** recipients. Original host ciphertext and all Wi-Fi/campus/enrollment recipient rules remain unchanged. Do not edit the retained original password entries for future account-password changes: the selected source is `shared/marcos-password.yaml`. Sharing a password deliberately shares its compromise/rotation scope; key-only SSH, password-required sudo and distinct host identities remain unchanged.

The user chose to **keep Bastion blocked**, without remote discovery. Before enabling its delivery, verify its distinct public recipient and runtime key location, then update its `fleet.secrets.ageRecipient`, the public `recipients` policy in `modules/access/administrators.nix`, and the exact shared rule in `.sops.yaml`; re-encrypt the shared file with SOPS under separate authorization. Never add it to ThinkPad's Wi-Fi file. `shared-password-recipients` checks the YAML against the Nix recipient policy; full canonical checks are mandatory before commissioning. Missing/unlisted recipients leave the candidate password binding undeclared and the account locked. Private identity/recovery/early-delivery review remains independent; a listed public key is not proof of decryption.

This is an **unactivated configuration change**. ThinkPad's existing review/readiness remains intact; Racknerd's early-decryption/recovery review remains false; Bastion's identity/recipient remains null. No installed account, service, remote host or storage was changed. The new shared source still needs separately authorized runtime delivery acceptance.
