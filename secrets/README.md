...
The shared account policy selects **`shared/marcos-password.yaml`** for `marcos` on all three hosts, now with declared early delivery on each. Bastion's separately authorized temporary bootstrap installed this password; its new-identity recovery and baseline early delivery remain unverified. No ThinkPad or Racknerd installed password changed.
...
| bastion | `hosts/bastion-tailscale.yaml` (enrollment); shared-password recipient verified | `/persist/var/lib/sops-nix/key.txt`; new-identity recovery/test declined |
...
The initial exact shared rule granted only **operator, ThinkPad and Racknerd**; the authorized Bastion follow-up below adds its independently verified distinct recipient. Original host ciphertext and all Wi-Fi/campus/enrollment recipient rules remain unchanged. Do not edit the retained original password entries for future account-password changes: the selected source is `shared/marcos-password.yaml`. Sharing a password deliberately shares its compromise/rotation scope; key-only SSH, password-required sudo and distinct host identities remain unchanged.

The user initially kept Bastion blocked without remote discovery. During the separately authorized 2026-09-11/12 follow-up, its new distinct public recipient/runtime location were verified and added to `fleet.secrets.ageRecipient`, the public policy in `modules/access/administrators.nix`, the exact shared `.sops.yaml` rule and the SOPS ciphertext. ThinkPad's Wi-Fi and all host-specific ciphertext remain unchanged. `shared-password-recipients` checks the YAML against the Nix policy; missing/unlisted recipients still lock candidate accounts. Public membership never substitutes for private identity/recovery/early-delivery review.

The baseline configuration remains **unactivated**. ThinkPad's existing readiness remains intact; Racknerd's identity review stays false. Bastion's temporary account/bootstrap is described below, not a deployed baseline. New-source early runtime delivery still needs separately authorized acceptance.

## Bastion provisioning checkpoint

**2026-09-11/12:** the operator authorized a distinct identity and shared-password recipient expansion after confirming console and existing backups. Strict SSH verified root-owned `0700`/`0600` identity storage and a public-canary roundtrip. Native `sops updatekeys` preserved the shared password, with original/new MAC and in-memory equality checks. Bastion then decrypted with only its own identity and installed/compared the temporary `marcos` password without exposing plaintext. The operator confirmed password sudo. [Full access/recovery evidence](../docs/hosts.md#bastion-authorized-staged-preparation--2026-09-1112) records the unchanged generation and retained root SSH.

The operator **declined a recovery copy/test for this newly created identity**. `identityReviewed` remains false; earlier backups cannot cover a key created afterward. No private identity was exported. The operator subsequently supplied `hosts/bastion-tailscale.yaml`; its encrypted `tailscale-auth-key` field and exact operator + Bastion recipients passed checks without decryption. The unactivated candidate selects ordinary root-only `0400` runtime delivery and enrollment-unit restart ordering. Native manifest validation checks the selected field, not the key's validity or private settings. Follow [Bastion key preparation](../docs/tailscale.md#bastion-auth-key-preparation); the OS recovery/migration gates remain unresolved.
