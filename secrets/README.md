...
| racknerd | `hosts/racknerd.yaml`, operator-supplied existing password hash via `sops edit`; enrollment file not yet created | `/persist/var/lib/sops-nix/key.txt`, authorized dedicated identity; root `0600`, parent `0700` | Canary passed; the operator attested custody but declined the recovery test. `identityReviewed` remains false until early decryption and recovery are verified |
