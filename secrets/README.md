...
| bastion | Shared-password recipient verified; enrollment file not supplied | `/persist/var/lib/sops-nix/key.txt`; new-identity recovery/test declined |
...
The operator **declined a recovery copy/test for this newly created identity**. `identityReviewed` remains false; earlier backups cannot cover a key created afterward. No private identity was exported. The exact `hosts/bastion-tailscale.yaml` rule permits only operator + Bastion, but the operator's choice to provision a key is not a supplied ciphertext file. Follow [Bastion key preparation](../docs/tailscale.md#bastion-auth-key-preparation); rollout remains disabled.
