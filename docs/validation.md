...
- The original empty-scaffold NixOS access assertion is no longer expected on current hosts: the selected public key remains supplied. SOPS ciphertext/binding is restored for ThinkPad; the other hosts have explicit null credential/identity blockers. No full real identity review is acknowledged. ThinkPad's operator-run private decryption/MAC and installed binding checks passed separately on 2026-09-10; canonical checks still never decrypt. No NixOS access assertion is disabled. Other non-bootstrap failures remain errors.
