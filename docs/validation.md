...
Racknerd, Bastion and Dino retain `ready = false` and remain absent from standard NixOS outputs. Their credential/identity, boot/migration/network/provider/NAS and deployment-trust blockers remain visible in `fleet`; Dino also has null original stateVersion and EFI NVRAM policy. `deploy.nodes` is still empty: ThinkPad is local-only and the remote-intent hosts are unready. No fake configurations or endpoints are exported.
...
- For Racknerd, Bastion and Dino, `just ready HOST`, building the absent standard host output, or forcing an unapproved `fleetConfigurations.HOST.config.system.build.toplevel` must fail. ThinkPad's build readiness must pass; deployment readiness still refuses its local-only intent. This is the desired safety distinction.
...
No real display manipulation, printer contact, secret decryption, activation, deployment or fleet-host operation occurred in this task. Mocks and build results do not replace separately authorized runtime acceptance.

## Not claimed / remaining acceptance

ThinkPad's earlier commissioning, real local build and operator-run two-boot acceptance are recorded above; the other three hosts remain uncommissioned. Evaluation fixtures are not built/booted installations. The subsequent maintenance candidate and these review fixes have not been activated or booted, and their checks perform no host, display, printer, credential or storage operations. No new runtime acceptance is claimed from mocks or derivation builds.

For the remaining hosts, resolve [the existing-host transition gates](hosts.md), run `just check`, `just ready HOST`, `just build HOST` and complete separately authorized first-boot acceptance. For ThinkPad, rerun those local checks and separately authorize maintenance activation/acceptance with the accepted generation retained. `just disk-plan HOST` must refuse every current host even after readiness; provisioning requires a separate reviewed fresh-install design. Runtime credential existence, actual hardware/initrd compatibility, by-id correctness, data-disk separation, network/provider behavior, service migrations, SSH trust/elevation and recovery must be verified on real machines. Those cannot be proven by pure evaluation.
