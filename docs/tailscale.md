...
[Current host status](hosts.md#current-status) is authoritative for policy-maintenance completion, client rollout and remaining acceptance. Policy application is not enrollment or traffic acceptance. The [2026-09-10 apply evidence record](validation.md#operator-reported-policy-apply-and-non-saving-verification--2026-09-10) distinguishes operator attestations from agent metadata-only checks.

Before application, the operator confirmed the intended tailnet, scopes, exclusive writer control, unused/uniquely correct fleet tags, independent administrative access and tested offline policy/DNS/state-backup recovery. The prior plan remains at `$TAILSCALE_STATE_DIR/reviewed-plan-71i6d10m/change.tfplan`; the applied plan is retained at `$TAILSCALE_STATE_DIR/change.tfplan`. After the apply report, the agent checked only tailnet binding, ownership/permissions (`0700` directory, `0600` state/plan files) and encrypted envelopes, without decryption, mutations or API access. Preserve these historical files and do not reuse the saved plan. The current checked policy differs from that historical applied evidence; during Bastion preparation the operator separately confirmed it is applied and verified, with independent admin access and the intended tag boundary. This task performed no API readback or apply. Any future live update still requires fresh review, planning and explicit authorization.
...
| racknerd / bastion | Old configurations, active daemon, `NeedsLogin`, no current tailnet/node ID. Live and backing directories `0700 root:root`, state files `0600 root:root` | Review retained state and its binding/backups before choosing enrollment; metadata does not prove state-file contents or identity equivalence |

OS readiness blockers and remote-deployment deferral are recorded in [current status](hosts.md#current-status). **Do not deploy the new baseline first to discover these facts.** Follow [host commissioning](hosts.md#access-and-state-migration-checklist--no-execution-authorized) with independent recovery. A successful evaluation cannot authorize or substitute for those transitions.
...
|---|---|---|
| `tag:fleet-thinkpad` | `tag:fleet-racknerd`, `tag:fleet-bastion` | TCP 22 and ICMP |
...
## Bastion auth-key preparation

[Authorized preparation](hosts.md#bastion-authorized-staged-preparation--2026-09-1112) verified the retained live/backing file and root-only modes. The daemon reports `NeedsLogin`; its old `tag:server` preference and saved `LoggedOut=false` are not usable authentication. Current preferences show no routing/exit-node, Serve, SSH, webclient, operator or connector configuration. The operator confirmed protected retained-state recovery and the current applied policy/tag boundary. The operator then reported completing the private provisioning below and supplied `secrets/hosts/bastion-tailscale.yaml`. Its encrypted field and exact operator + Bastion recipients pass structural checks; the unactivated candidate now selects root-owned `0400` `/run/secrets/tailscale-auth-key`, ordinary runtime delivery and `fleet-tailscale.service` restart ordering. No key was decrypted or submitted. OS commissioning remains independently blocked by the declined new-age-key recovery and timer-state migration.

1. In **`Td9HdopnWQ11CNTRL`**, privately create a short-lived auth key: **Reusable off**, **Ephemeral off**, only **`tag:fleet-bastion`**, **Preauthorized off**. Approve only the intended device after first contact if required. Never reuse another host's key or run `tailscale up` manually.
2. In a private terminal and locked development shell, use a trusted editor without AI/cloud integrations, swap or backups. The exact rule already contains only the verified operator + Bastion recipients. Set the `tailscale-auth-key` YAML field and save encrypted output:

   ```sh
   sops edit secrets/hosts/bastion-tailscale.yaml
   ```

3. Run `just secret-check` before staging and report only success plus confirmation of the key settings. No key or decrypted diagnostic belongs in chat. The supplied file is now selected and Bastion's rollout/oracle updated; see [candidate validation](validation.md#bastion-auth-key-candidate--2026-09-12). Repeat private provisioning only for an explicitly requested replacement, such as an expired key before rollout. Identity recovery, migration review and activation authorization remain independent gates.
...
These tests cannot establish real key validity/decryption, tailnet identity/plan entitlement, API policy acceptance, tag cardinality, firewall behavior, credentials, networking, hardware boot or restore. Read-only access/scopes, complete diff review and offline backup recovery are now operator-confirmed, separately from these tests. Exclusive policy-writer control, unused/uniquely correct fleet-tag assignments and independent administrative recovery access were operator-confirmed for that earlier maintenance task. The minimal write scopes and fresh exact policy-only diff were subsequently operator-confirmed, followed by a successful apply report. These attestations are not blanket authorization for later changes. The report is evidence of API acceptance, not an agent-observed readback or real allowed/denied flow test. The operator subsequently confirmed no-drift verification, post-apply independent recovery and checked protected backups. ThinkPad's and Bastion's host-state/policy review and key provisioning are operator-reported; their candidate rollouts are enabled, not activated. Key validity/settings, dedicated-identity runtime decryption, device approval and enrollment remain runtime gates, not facts established by encrypted payload checks. Bastion's ciphertext is supplied and selected, but its new-identity recovery and timer-state migration gates stay open. Its boot/network/NAS preflight flags are reviewed; OS readiness remains false. Deployment and live enrollment are not complete.
