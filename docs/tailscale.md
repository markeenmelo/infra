...
3. Run `just secret-check` before staging and report only success plus confirmation of the key settings. No key or decrypted diagnostic belongs in chat. Selecting **Provision now** is not proof the file was created; no declaration should point at an absent file. Once supplied, select the ordinary root-only runtime secret, update only Bastion's rollout/oracle, and validate. Identity recovery, migration review and activation authorization remain independent gates.
