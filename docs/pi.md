...
Shared public configuration lives in `modules/agents.nix`, with checks/assets under `modules/agents/`. ThinkPad consumes its native Home Manager contribution; Bastion selects the matching NixOS `agents` capability without desktop/HM. The extension set is declared directly in Pi's managed `settings.packages`: pinned `npm:` specs for published packages plus three immutable Nix store entries (hash-pinned `pi-review`, the official RTK hook and the repository's `empty-args-retry.ts`). No vendored `package.json`, npm lock, patch set or Nix npm build exists anymore. Changes require a separately authorized system/Home Manager activation and a **new Pi session** afterward. This work does not activate or reload a running session. Do not overwrite managed settings using `pi install`, `/curator` or settings-writing commands; change the declaration instead. Credentials, sessions, trust decisions, artifacts and Pi's installed package trees remain private mutable home state.

Bastion's temporary [administration workspace](reinstall.md#bastions-temporary-administration-workspace) keeps this public configuration with its own stable **Pi 0.75.4 / RTK 0.41.0**, as explicitly selected instead of backports. Native user tmpfiles supplies non-forcing home links to `/etc/fleet-agents`; existing files are not overwritten, and stable `/etc` targets follow the system generation. Inspect links/collisions on first login. The private persisted `marcos` home retains auth, sessions, npm trees and RTK state; none is copied from ThinkPad.

**Native-version limitations:** 0.75.4 does not implement `defaultProjectTrust`, so retaining `"ask"` in JSON does not enforce it: use only operator-reviewed checkouts. Its actual retry classifier also retries prefixed quota errors within native bounds; modern Pi rejects them. Local extension loading/rewriting is tested on both pins, but mutable npm installation, provider login and real workflows remain unaccepted. Herdr and standalone Claude/Codex CLIs remain absent; Cursor CLI is included only as the existing Pi provider dependency.
...
| RTK | Native CLI 0.41.0 stable / 0.47.0 unstable; hook v0.47.0 | Official source/hash-pinned hook, tested against both native CLIs; no binary backport |
| `empty-args-retry.ts` | Repository source / Nix store path; separately tested Pi 0.75.4 / 0.85.1 contracts | Converts empty-argument calls for active tools with required schema properties into a transient provider error; uses Pi's bounded agent retry, not argument repair |
