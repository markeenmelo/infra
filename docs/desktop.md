...
| Pi 0.85.1 | Native HM settings and package; default `openai-codex/gpt-6-astra`; [managed pinned extensions](pi.md), Codex usage/search, Cursor provider, review, rewind, RTK and empty-argument retries. Subagents, plan mode, LSP and direct-fetch tools were removed. Project trust **ask**, rewind restoration **ask**, no automatic restore on resume/fork/clone. OAuth/API credentials stay mutable and private in the existing home |
...
The current Pi extension set needs only Cursor CLI as a provider dependency; `cursor-cli` is the single unfree package name allowed on the desktop. Standalone Cursor settings/hooks were not imported. Pi's store-backed settings cannot persist `/settings`, model saving or `pi install`; the extension set is declared in Pi's managed `settings.packages` (see [Pi package management](pi.md#package-management)), so modify the declaration instead of letting an updater overwrite store-backed files. The former Herdr Pi extensions were removed with the npm build, so Pi no longer emits Herdr pane state; the Herdr program itself is unchanged. The installed model catalog and deployed settings confirm `openai-codex/gpt-6-astra`; provider authentication and the `npm:` extension runtime still require a new Pi session for application-level acceptance.
