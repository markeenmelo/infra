...
A Hyprland + Noctalia desktop for `marcos`. Display/layout/shell policy is authored from observed hardware; the user subsequently authorized reusing the current **application and shell preferences** listed below. The other three hosts remain headless and have no Home Manager evaluation. ThinkPad is commissioned and remains local-only. The maintenance generation is now deployed and booted, but limited runtime review exposed an initial output-publication race; the branch's guard is not yet activated and full maintenance acceptance remains open. Existing storage, access and recovery protections still apply.
...
The generated policy and live **dry-run** are validated without issuing a modeset. On the deployed maintenance generation, a cold docked login had zero Hyprland configuration errors but retained the static side-by-side 8-bit layout with eDP-1 active while the lid was closed; the already-running watcher had synchronized before the external output was published. The branch now adds the subscriber-first settling delay and an isolated regression, but that guard has not been activated or modeset-tested. Follow-up acceptance must test cold docked/undocked login, hotplug, config reload, lid close/open, last-external removal while closed, HDR content/SDR mapping and `hyprctl configerrors`. Preferred mode means the EDID-designated native mode, not the non-native 3840×2160 timing or lower-resolution 144 Hz modes also advertised by this ultrawide. Ten-bit alone is not HDR; OLED VRR flicker is not eliminated by a compositor setting. Do not assume unplugging an active eGPU is safe or seamless.
