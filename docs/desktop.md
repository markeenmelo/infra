...
|---|---|
| Ghostty 1.3.1 / Herdr 0.8.2 | Native HM modules; OLED Graphite, JetBrains Mono, opaque terminal, Herdr as command; Ctrl+A prefix, Zsh default, restored sessions/agent indicators. Disable Herdr release fetching; review its mutable session/integration state before reuse |
| Zed 1.17.2 | Native HM settings/theme/LSPs; current OLED Graphite preferences, telemetry/update disabled. Nix formatting uses the repository's official `nixfmt -`, not Alejandra. Settings are declarative, not writable through Zed's settings UI |
...
For campus provisioning, follow the [secure SOPS editor workflow](../secrets/README.md). Add encrypted **`seneca-identity`** and **`seneca-password`** scalars to the existing ThinkPad YAML without losing its existing fields/recipients, then set `fleet.wifi.senecaSopsFile` to that reviewed encrypted file in a ThinkPad module. Do not use YAML literal blocks with trailing newlines, shell commands containing credentials, dummy values, chat or plaintext files in the repository. Run `just secret-check` before staging; the actual manifest check validates selected encrypted keys without decryption. This is a credential-provisioning step, not a review-flag bypass.
