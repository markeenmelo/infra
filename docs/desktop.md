...
5. The original desktop review and two-boot commissioning acceptance are recorded in [hosts.md](hosts.md#first-boot-acceptance-2026-09-10-utc). The maintenance generation has since booted, but its limited runtime checks do not certify the dependency, printer or HDR/output-policy change; cold docked startup exposed the unapplied monitor rules described above. Keep `fleet.desktop.reviewed` tied to the genuine review rather than toggling it to disguise an incomplete update. `just ready thinkpad` and `just build thinkpad` are additional local checks, not activation permission.
