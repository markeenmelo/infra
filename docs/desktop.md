...
- `modules/desktop/thinkpad.nix`: selects `marcos`, deliberate `home.stateVersion = "26.05"`, the observed internal-panel mode, and the EDID-gated hotplug/lid output policy. This is not a recovered historical Home Manager stateVersion and must not follow future package updates.
...
| Pi 0.85.1 | Native HM settings and package; default `openai-codex/gpt-6-astra`; pinned extension manifest/lock with pi-codex-api 0.4.0 and Herdr hook, project trust **ask**, rewind restoration **ask**, no automatic restore on resume/fork/clone. OAuth/API credentials stay mutable and private in the existing home |
...
- Connected externals are sorted into one top row and use each EDID's preferred mode at scale 1. The internal panel is centered beneath the full row. A non-mutating open-lid dry-run placed the external at `(0,0)` and internal at `(1600,1440)`; a later closed-lid dry-run kept the external rule and emitted `eDP-1,disable`.
...
- Closing the lid disables eDP-1 only while at least one external is active. Opening it restores the centered layout; disconnecting the last external while closed restores eDP-1 to avoid a blank session. A locked watcher handles monitor add/remove events, and locked lid binds trigger the same idempotent synchronization.
...
The generated policy and live **dry-run** are validated without issuing a modeset. Post-activation acceptance must still test cold docked/undocked login, hotplug, lid close/open, last-external removal while closed, HDR content/SDR mapping and `hyprctl configerrors`. Preferred mode means the EDID-designated native mode, not the non-native 3840×2160 timing or lower-resolution 144 Hz modes also advertised by this ultrawide. Ten-bit alone is not HDR; OLED VRR flicker is not eliminated by a compositor setting. Do not assume unplugging an active eGPU is safe or seamless.
...
5. The original desktop review and two-boot commissioning acceptance are recorded in [hosts.md](hosts.md#first-boot-acceptance-2026-09-10-utc). They do not certify this later dependency, Bluetooth or HDR/output-policy change. Keep `fleet.desktop.reviewed` tied to that genuine review; do not toggle it to disguise a failed update. `just ready thinkpad` and `just build thinkpad` are additional local checks, not activation permission.
6. During a separately authorized maintenance activation, retain the previous Limine generation and test the new bar/Bluetooth/Pi defaults plus cold docked/undocked login, hotplug, lid close/open, last-external removal while closed, HDR/SDR rendering and `hyprctl configerrors`. Check `herdr status` before replacing any still-running 0.8.2 server or risking its panes, verify the 0.9.0 Pi agent indicator, and confirm Zed still waits for explicit project-search submission. Recheck Bitwarden startup/browser integration, KDE Connect pairing/transfer, password login, lock-before-suspend, audio/portals and console recovery. Review user/system journals without publishing sensitive desktop content.
