...
- Closing the lid disables eDP-1 only while at least one external is active. Opening it restores the centered layout; disconnecting the last external while closed restores eDP-1 to avoid a blank session. A locked watcher handles monitor add/remove and `configreloaded` events (reload discards runtime monitor rules), and locked lid binds trigger the same idempotent synchronization.
- Monitor JSON/row decoding completes before any rule is applied. Command, EDID decoding, hardware-read and socket failures stop processing instead of silently selecting SDR, continuing to disable eDP-1 or retrying. Already applied rules are not rolled back; diagnose failures before starting a new watcher/session.
...
The generated policy and live **dry-run** are validated without issuing a modeset. Post-activation acceptance must still test cold docked/undocked login, hotplug, config reload, lid close/open, last-external removal while closed, HDR content/SDR mapping and `hyprctl configerrors`. Preferred mode means the EDID-designated native mode, not the non-native 3840×2160 timing or lower-resolution 144 Hz modes also advertised by this ultrawide. Ten-bit alone is not HDR; OLED VRR flicker is not eliminated by a compositor setting. Do not assume unplugging an active eGPU is safe or seamless.
