...
- The deliberately selected `desktop` bundle (originally named `hyprland`) is the shared destination for compositor, greeter, shell, apps and peripherals; its integration module owns the Home Manager bridge, matching-class composition and supported-track rejection ([ADR 0008](0008-native-desktop-and-kernels.md)). The fleet evaluator knows no desktop name. Agent policy (`modules/agents.nix` and adjacent checks/assets) now contributes only to ThinkPad's HM desktop. The temporary native NixOS agent capability and its server-only fixtures were removed on 2026-09-13 when the operator selected the MacBook for administration. Servers have no HM, editor or agent workspace; the shared server concern owns that policy/check.
