...
- Status: accepted design; amends [ADR 0005](0005-existing-headless-baseline.md)'s VPN deferral; tooling track amended by [ADR 0010](0010-native-devenv.md); ThinkPad unactivated candidate enabled, other rollouts disabled
- Date: 2026-09-10
...
- Plain OpenTofu HCL with a separate policy file under `tofu/tailscale/` — no Terranix layer, new flake inputs, host package mixing or second policy publisher. Unstable development tooling supplies OpenTofu and a Nix-pinned offline provider mirror, with exact native/flake wrapper parity per [ADR 0010](0010-native-devenv.md). `tofu/tailscale/tailnet.json` is the single shared public tailnet identity; environment confirmation cannot retarget the provider or state wrapper. Initially manage only the whole policy and MagicDNS; existing resources must be imported/reviewed; destruction/reset protections and native publisher/editor conflicts stay explicit.
