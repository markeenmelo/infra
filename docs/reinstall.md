...
- Confirmed order: **Bastion → Racknerd → ThinkPad**. ThinkPad stays usable until both servers are accepted and Bastion independently provides the repository, tools and reviewed credentials needed for the final reinstall. Bastion now has two-boot acceptance; the subsequent Racknerd task has its own explicit authorization and pre-install evidence. ThinkPad operations remain unauthorized.
