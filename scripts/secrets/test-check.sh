...
set -euo pipefail
repo=$PWD
guard="$repo/scripts/secrets/check.sh"
work=$(mktemp -d)
...
(cd "$work" && bash "$guard")
for variant in plaintext recipient missing-mac unsupported-backend invalid-yaml multiple-documents duplicate-key merge-key nested-merge shared-plaintext shared-extra-key; do
  ...
  case "$variant" in
    ...
    multiple-documents)
      { printf 'plain: TEST-ONLY-NOT-A-SECRET\n---\n'; dd if="$repo/secrets/hosts/thinkpad.yaml" status=none; } > "$file"
      ;;
    duplicate-key)
    ...
    merge-key)
      ...
      ;;
    nested-merge)
      awk '{ print } /^sops:$/ { print "    <<: {mac: TEST-ONLY-NOT-A-SECRET}" }' "$repo/secrets/hosts/thinkpad.yaml" > "$file"
      ;;
  esac
  if (cd "$work" && bash "$guard") >"$work/result" 2>&1; then
