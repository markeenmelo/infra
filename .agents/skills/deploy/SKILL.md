...
3. Run `bash scripts/ready.sh HOST deploy`. Inspect `nix eval --json .#deploymentPlan | jq .`; resolve all placeholders. Build with `just build HOST` if prebuilding is appropriate. Review runtime secrets/signing keys separately: pure evaluation cannot verify them.
