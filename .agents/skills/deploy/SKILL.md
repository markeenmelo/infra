...
Read `../../../AGENTS.md`, `../../../modules/deployment.nix`, `../../../scripts/ready.sh`, `../../../docs/hosts.md`, `../../../docs/operations.md#deployment-and-recovery` and the target's metadata. The existing servers currently have root-only SSH; the headless target disables root SSH and removes Tailscale, so a separately authorized staged access/routing/credential transition must precede activation. Dino opts into deploy-rs only after commissioning; thinkpad stays local-only. Require real commissioned NixOS, SSH account/endpoint/port, verified host key, working elevation, closure trust, backups and independent recovery access. Use `nix-research` after deploy-rs/Nix API changes and inspect the **locked** `deploy --help`.
