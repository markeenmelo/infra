...
2. Run `devenv tasks run repo:check` before remote contact. Inspect `devenv tasks run repo:inventory`; compare the target's track/revision and actual `nixpkgsPath` with intended input. `validation.nix` independently enforces the host-track policy; never change the oracle merely to bypass an unexpected track.
