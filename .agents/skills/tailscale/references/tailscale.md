...
[Current host status](../../fleet-operations/references/hosts.md#current-status) is authoritative for policy-maintenance completion, client rollout and remaining acceptance. Policy application is not enrollment or traffic acceptance. The [2026-09-10 apply evidence record](../../validate/references/validation.md#operator-reported-policy-apply-and-non-saving-verification--2026-09-10) distinguishes operator attestations from agent metadata-only checks.
...
The public tailnet ID **`Td9HdopnWQ11CNTRL`** is recorded in [`tofu/tailscale/tailnet.json`](../../../../tofu/tailscale/tailnet.json). Both OpenTofu and the wrapper read this single source; optional environment confirmation cannot retarget it. An enabled, reviewed candidate adds the daemon/enrollment unit, UDP 41641 and the existing backing-state bind; disabled candidates preserve backing state without an active daemon. Consult current status before selecting a host. `nix eval --no-update-lock-file --json .#tailscalePlan` reports separate rollout prerequisites; `nix eval --no-update-lock-file --json .#fleet` still reports OS commissioning.
...
The locked development shell supplies unstable-track OpenTofu **1.12.6** with only Nix-pinned Tailscale provider **0.29.2**, offline. `.terraform.lock.hcl` records that packaged linux_amd64 artifact. Init reports the locally mirrored provider as **unauthenticated** because it is not an upstream signed release archive: trust comes from the pinned Nix source/build/cache verification. Do not silently replace it with a registry binary or regenerate its checksum on unexplained mismatch. New platforms and provider updates need explicit review/tests. Native devenv has its own development-only lock; manual validation requires the same unstable Nixpkgs and exact provider wrapper as the flake. Production input aliases/follows changed, but their source revisions and host package tracks are preserved.

A Nixpkgs update can change the local mirror checksum **without a provider version change**: withPlugins hashes a shell entry-point whose Bash/provider store paths are pinned. On mismatch, compare old/new source/vendor hashes, derivations and actual wrappers before any lock change. Generate the supported-platform checksum only from the locked `libexec/terraform-providers` filesystem mirror with `tofu providers lock -fs-mirror=... -platform=linux_amd64 registry.opentofu.org/tailscale/tailscale`, in a disposable versions-only directory with no operator credentials/state. Review the single resulting h1 update; never switch to a registry binary or disable readonly verification. The [dated checksum review](../../nix-research/references/research.md#nix-built-provider-checksum-refresh--2026-09-13) records this batch's unchanged-source rebuild.
...
6. **Only after explicit authorization**, `tailnet apply` inside the private shell applies that saved plan after an exact tailnet-ID confirmation. It changes the live tailnet, not NixOS. The plan is retained; archive it securely outside Git before making the next plan. Do not reuse old plans after policy/emergency changes; replan against refreshed state and serialize all applies. Never wire apply into [full manual validation](../../validate/SKILL.md), shell entry, a Nix build or NixOS activation.
