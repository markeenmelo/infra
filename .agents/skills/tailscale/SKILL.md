...
ThinkPad preserves its reviewed existing node and persisted `/var/lib/tailscale` state; no enrollment key is tracked, declared or consumed. A fresh installation without that state comes up in `NeedsLogin` and stays there. Enroll it deliberately on the host with a runtime single-use key, only while the daemon reports `NeedsLogin`, and verify the applied tag in the admin console afterwards — no code checks it. Reconnect a stopped identity with a plain `up`, never a forced reset that loses preferences.
