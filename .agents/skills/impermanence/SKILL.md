...
7. Stage and run `devenv tasks run repo:fmt`, `devenv tasks run repo:check`. Inspect the changed inventory and assertions for duplicates or missing mounts; build affected commissioned systems if possible. For changes spanning reusable modules, validate both tracks.
