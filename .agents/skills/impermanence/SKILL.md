...
Read `../../../AGENTS.md`, `../../../modules/storage/persistence.nix`, `../../../modules/logging.nix`, `../fleet-operations/references/hosts.md`, `../storage-disko/references/bootstrap.md` and the owning feature. Research current impermanence/systemd initrd behavior via `nix-research` if touching boot/mount APIs or updating inputs. Obtain a migration backup and state ownership facts.
...
7. Follow [manual validation](../validate/SKILL.md), including ciphertext inspection before staging/evaluation and full local checks. Inspect the changed inventory and assertions for duplicates or missing mounts; build affected commissioned systems if possible. For changes spanning reusable modules, validate both tracks.
