...
Read `../../../AGENTS.md`, `../../../modules/storage/persistence.nix`, `../../../modules/logging.nix`, `../../../docs/bootstrap.md` and the owning feature. Research current impermanence/systemd initrd behavior via `nix-research` if touching boot/mount APIs or updating inputs. Obtain a migration backup and state ownership facts.
...
3. Review this checklist:
   ...
   - User state: documents, game saves, credentials and homes. Workstations intentionally persist `/home`; do not redundantly bind each child path. Server home state is not persistent by default.
...
4. Add state beside its owning feature through `environment.persistence."/persist".directories` or `.files` in its deferred NixOS value. Specify directory owner/group/mode where defaults are inappropriate. Runtime secrets stay outside the Nix store; declarations do not deliver their contents.
...
7. Stage and run `just fmt`, `just check`. Inspect the changed inventory and assertions for duplicates or missing mounts; build affected commissioned systems if possible. For changes spanning reusable modules, validate both tracks.
