...
3. Run **`devenv tasks run repo:check`** once for the final coherent candidate (and for deployment preflight). [Canonical command and check inventory](../../../docs/validation.md#canonical-command) are maintained in `docs/validation.md`; native `devenv.nix` defines the dependency order; use the default `before` mode, never `--mode single`. Do not substitute targeted checks for this gate or run real `devenv shell tailnet` or `tailnet-sops` operations as validation.
