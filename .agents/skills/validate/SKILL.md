...
3. Run **`just check`**, whose order is:
   ...
   - Tailscale additionally checks disabled real-host rollout, both-track synthetic preserve/auth-key composition and unsafe-review/tag/secret/override rejection, actual CLI flag support, mocked reconciliation/operator-wrapper guards, exact default-deny policy regressions, OpenTofu schema/mock plans and temporary local-only state/plan encryption. `terraform_data` is the only synthetic apply; no real provider/host is contacted. `just tailscale-inventory` reports feature blockers separately from OS readiness. Real `just tailnet` operations are never validation dependencies.
