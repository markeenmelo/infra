...
1. Inspect `git status --short`/diff. Ensure every non-entry-point `.nix` file is a top-level module under `modules/`. Stage intended new files (Git flakes ignore untracked files); do not stage unrelated changes or secrets. Keep the lock unchanged unless updates are in scope.
...
3. Run **`just check`**, whose order is:
   ...
   - `nix eval --no-update-lock-file --json .#validation`: every real composition report and package/`/etc`/initrd derivation, independent host-track mapping, actual package-source and locked-branch assertions, exact capability-subset fixtures, both-track UEFI/BIOS fixtures and negative disk tests.
   - `nix flake check --no-update-lock-file -L`: standard output evaluation and built check derivations, including upstream deployment schema/activation checks and both-track non-empty smoke payloads.
