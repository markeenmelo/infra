...
6. **Agents.** Pi's extensions are pinned `npm:` specs in its managed settings, without managed extension defaults. Keep the pinned set and Pi's core provider, model, thinking level and changelog settings, and keep project trust set to ask.
