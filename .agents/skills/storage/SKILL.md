...
Read the host's `modules/hosts/<host>/disko.nix`, `modules/storage/persistence.nix` and `modules/storage/limine.nix`. `persistence.nix` owns the `fleet.installation.osDevice` option and its whole-disk validation, and puts disko, the tmpfs root, the empty-pool guards and the `/persist` `neededForBoot` flag into `base`; a host file declares only its disk.
...
- `/persist` needs an explicit `neededForBoot`; NixOS already mounts `/` and `/nix` in the initrd, so the report shows them as `false`. `/nix` must be a real mounted subvolume, never an impermanence binding.
...
The mode creates **empty** `0600` placeholders for missing `known_hosts` and `disko.sha256`. It reports these as **BLOCKED** with troubleshooting steps and returns nonzero; generated files are kept for the next run. It creates `extra-files/persist/var`, `var/lib` and the age directory only for a host selecting secrets, but never generates an age identity: import the reviewed identity manually with the modes above. Existing `var` and `var/lib` payload directories must be `0755` on every host, even those without selected secrets, matching the installation branch.
...
| Unsafe bootstrap-key ancestors | Keep the existing key under root/operator-owned parent directories without group/other write access, such as a protected home directory. Shared `/tmp` ancestry is refused even if the key itself is `0600`; do not change shared directory permissions to bypass this check. |
