...
[Research and pins](../nix-research/references/research.md#dendritic-concern-ownership-refactor--2026-09-11) distinguish the upstream pattern from local choices. Dendritic itself does not mandate flake-parts, import-tree, a fleet schema, or a separately selectable module name for every file.

## Prerequisites

Read `../../../AGENTS.md`, `../../../README.md`, `references/adr/0001-dendritic-composition.md`, `../../../flake.nix`, `../../../modules/fleet.nix` and every affected concern/consumer. Read [retained implementation rationale](references/implementation-notes.md) for constraints formerly explained in code comments. For Home Manager read `../../../modules/desktop.nix`; for tests read `../../../modules/validation.nix` and the feature's checks. Use `nix-research` for dependency-sensitive decisions.
...
6. **Keep tooling and tests with their owner.** Native `devenv.nix` assembles the unstable toolbox and three explicit scaffold/install/deploy tasks; bodies live in `scripts/devenv/`, without an automatic repo DAG or production import. The explicit development entry-point exception is documented in [ADR 0010](references/adr/0010-native-devenv.md). Keep `modules/` Nix-only; executable sources/tests live in `scripts/<concern>/`, static data in `assets/<concern>/`. Update all callers and repository-root calculations after moves. The small validation harness owns typed fixture assembly and independent output/track inventories, not feature assertions. Owners contribute `fleet.validation.fixtureModules`, `hostChecks`, `flake.validation` and `perSystem.checks`; fixture facts remain synthetic and capability-scoped.
...
5. Follow [validate](../validate/SKILL.md): ciphertext guard before staging/evaluation, manual formatting/static/tool parity/full flake checks, and readiness/builds for affected commissioned hosts. Both tracks cover reusable infrastructure; supported-track and rejection checks cover desktop. Review the complete diff and report exact evidence and unresolved runtime acceptance. Targeted probes or read-only audits do not replace canonical checks.
