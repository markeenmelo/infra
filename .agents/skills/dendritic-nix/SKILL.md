...
6. **Keep tooling and tests with their owner.** Features contribute `perSystem.devPackages` from that evaluation's stable `pkgs`; tooling only assembles the shell. Feature scripts/assets stay nearby; update all callers and repository-root calculations after moves. The small validation harness owns typed fixture assembly and independent output/track inventories, not feature assertions. Owners contribute `fleet.validation.fixtureModules`, `hostChecks`, `flake.validation` and `perSystem.checks`; fixture facts remain synthetic and capability-scoped.
...
- Inputs and deterministic sorted discovery stay in `flake.nix`; no second Nix entry point. The existing traversal includes `_`-prefixed files and `default.nix`, ignores non-Nix assets and does not follow symlinks. A reference repository's exclusion rules do not apply here.
