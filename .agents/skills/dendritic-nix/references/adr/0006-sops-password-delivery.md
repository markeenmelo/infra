...
- **Shared password (amendment):** the user selected the existing ThinkPad `marcos` password for all three hosts, authorizing only local in-memory extraction/re-encryption. A separate password-only YAML (`secrets/shared/marcos-password.yaml`) supplies the common account policy, MAC/equality-verified without exposing plaintext. It initially permitted verified operator, ThinkPad and Racknerd recipients; the explicitly authorized 2026-09-12 recipient-only update adds Bastion's new distinct identity, retaining the original encrypted password payload and every host-specific file. Typed `fleet.secrets.ageRecipient` records each known host's public identity; missing/unlisted recipients or a missing key-file binding retain the locked candidate account and block commissioning. A built check compares the YAML recipients to the explicit public Nix policy; no YAML parser enters Nix evaluation. New membership requires a verified distinct recipient deliberately added to both ciphertext and policy; Bastion now meets this requirement. Shared credentials widen compromise/rotation scope and permit neither password SSH nor passwordless sudo.

## Secret-free server amendment — 2026-09-13

Only ThinkPad retains interactive password/Wi-Fi/enrollment secret delivery. Both servers use the explicitly approved locked-password `deploy` account and select no SOPS secrets. The deferred SOPS options remain available through access, but secret-free hosts must have null age path/recipient and no active identity-review metadata; the native module emits no secret-install activation in that case. Do not require or stage an unused server age key. Selecting a future secret reinstates genuine identity/recipient/recovery blockers; no automatic generation or assertion bypass is introduced.

The operator chose private, operator-run ciphertext rotation. Current public rules/ciphertext and their matching recipient oracle remain unchanged until the [coherent revocation procedure](../../../../../secrets/README.md#retire-server-recipients--operator-run-pending) is completed. That procedure removes current/previous server recipients and rotates data keys, preserving secret values and off-host recovery identities. It cannot revoke historical ciphertext or historical password knowledge; actual credential rotation is separate.
