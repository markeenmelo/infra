...
Read `../../../AGENTS.md`, `../../../flake.nix`, `../../../flake.lock`, `../../../docs/operations.md#input-updates`, and `../../../docs/research.md`. Start with a clean/reviewed working tree and known rollback commit. Enter `devenv shell`; see [bootstrap and lock synchronization](../../../docs/development.md#locks-and-architecture). Invoke `nix-research` before choosing branch/API changes; upstream support status must be checked anew.
...
3. **Unstable update (alternative scope):** inspect notable NixOS/module, driver, kernel, desktop and gaming changes since the pin. Keep the required `nixpkgs-unstable` branch rather than silently switching to `nixos-unstable`. Run:
   ...
   ```
   Focus on `thinkpad` and developer tooling; stable and unrelated dependency source pins must not move. Synchronize the native `devenv.lock` Nixpkgs node from the flake's `nixpkgs` node. Research native CLI/module and OpenTofu/provider compatibility; unversioned devenv source updates use `devenv update devenv` and remain separate from production source updates.
