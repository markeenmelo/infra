...
4. Existing `ready`, `build`, `disk-plan`, `deploy`, `tailnet` and `tailnet-sops` bodies live in `../../../scripts/devenv/`; `devenv.nix` registers them with `builtins.readFile`. They are not task replacements. Readiness/build/plan construction are local; raw `deploy` is not a guarded full preflight. Use the matching deployment/storage/Tailscale skill before operator commands.
