...
The manifest is a separate owned `0600` JSON file outside the staging tree, checkout and store. Its exact fields are `host` (selected fleet name), `machineId` (reviewed nonzero 32-character lowercase hex ID), `sshHostFingerprint` (installed ED25519 `SHA256:` fingerprint) and `ageRecipient` (installed dedicated age public recipient). Obtain these bindings from independently verified, host-labelled recovery records—not by blessing whichever staging directory was supplied. The helper compares the staged machine ID, derives the SSH public key from its private half and checks the staged public file/fingerprint, and derives the age recipient without printing private material. Before any target contact it snapshots staging and the manifest into a private temporary directory, rechecks that snapshot, and installs only that snapshot. This catches mixed-up host bundles; it does not establish the provenance or recovery quality of the operator-reviewed manifest.
