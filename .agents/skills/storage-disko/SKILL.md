...
Read `../../../AGENTS.md`, `../fleet-operations/references/hosts.md`, `references/reinstall.md`, `../dendritic-nix/references/adr/0003-storage-and-impermanence.md`, and the matching `../../../modules/hosts/<name>/disko.nix`. Current candidates use fresh per-host layouts. Only actually reviewed hosts expose provisioning outputs; read the authoritative dated host status for completed installs and remaining gates, never infer readiness or execution permission from this skill. `../dendritic-nix/references/adr/0005-existing-headless-baseline.md` records the superseded adoption design, not retained modules. Obtain actual target inventory and verified recovery. Use `nix-research` for locked disko/script/boot APIs and both tracks.
...
4. Declare native partitions and firmware settings directly in `modules/hosts/<host>/disko.nix`, still a top-level flake-parts module; no generic storage builder, old/new toggle or conventional host import roots. Use nullable `fleet.installation.osDevice` and fresh `storageReviewed` in the existing installation metadata. Each layout owns its guards and imports upstream disko/shared Limine policy. BIOS requires FAT boot plus first 1 MiB EF02; UEFI uses an ESP. Test the chosen sizes and review actual boot headroom. Unreviewed candidates remain blocked. Racknerd alone has a researched PCI by-path exception because no serial/by-id exists; verify VM, slot, capacity and current use rather than treating topology as a disk serial. Other layouts still require by-id. Both forms reject partition suffixes.
...
6. Design persistence and provisioning together: tmpfs root is mounted by disko; `/nix` and `/persist` must be persistent and early-mounted. Check firmware/bootloader behavior and keep Limine BIOS embedding on the same reviewed whole disk, with the first EF02 partition.
7. Complete [manual validation](../validate/SKILL.md), checking ciphertext before staging/evaluation; inspect the host's `fleet` report and filesystem/device expressions. Once all commissioning prerequisites are real, `devenv shell disk-plan HOST` builds the script **without executing it**. Read every destroy/format/mount command and device reference in `result-disko-HOST`.
...
9. Stop and report the reviewed plan and remaining prerequisites. Only separate explicit authorization permits the [host:install task](../devenv/references/development.md#separate-destructive-installation) or a separately reviewed installer-local operation. The task requires exact disk/identity/plan-hash confirmation, private installed-identity staging, a console-pinned root live installer and fresh unmounted-device checks; it uses prebuilt closures and only disko/install phases. No kexec, reboot, automatic hardware facts or host-key copying. Follow with mount/identity/boot inspection and separately authorized first-boot acceptance.
