...
**Updated execution scope:** Bastion's authorized NVMe install and two boots are accepted. Starting at `0cece65`, the operator separately authorized **Racknerd's exact single-disk fresh install through nixos-anywhere, new identities, signed/root-only trust and two boots**. [Authoritative dated status](../../fleet-operations/references/hosts.md#current-status) records reviews and remaining validation/runtime gates; ThinkPad stays unready and untouched. Each host composes its own `disko.nix`; no old/new storage framework remains. The 2026-09-13 follow-up removes Bastion's temporary workspace/home persistence and Racknerd's editor from the candidate; the operator will use the already-prepared MacBook instead. No deployment or data cleanup was performed. [ADR 0003](../../dendritic-nix/references/adr/0003-storage-and-impermanence.md) governs storage. Never activate these fresh layouts over old mounted disks, and do not broaden the [runbook](bootstrap.md#storage-and-installation) beyond the current task authorization.

## Bastion operator reinstall — 2026-09-13

The operator reports Bastion is now booted from a NixOS live USB and explicitly requested an **actual OS-reinstall command to run themselves**, not a deploy-rs update. No target was contacted and no installation/test was executed in this follow-up. PR fixes and secret-free server changes are committed but **not validated**, per the instruction to wait for test authorization; earlier batch results do not cover them. Do not treat the command below as a smoke test: after its gates pass it erases the OS disk, including old OS generations, homes and on-disk credentials. Boot rollback cannot undo that erasure.
...
- Confirmed order: **Bastion → Racknerd → ThinkPad**. Both servers now have two-boot acceptance. The operator selected their already-prepared MacBook for the final reinstall, superseding the Bastion workspace prerequisite. Independent repository/lock, both reviewed administrator private keys (or an explicitly accepted recovery subset), operator age identity, signers and recovery must remain accessible outside ThinkPad's future wipe; MacBook readiness is operator-reported, not tested here. ThinkPad operations remain unauthorized.
...
Implemented directly in `modules/hosts/bastion/disko.nix`, `modules/hosts/racknerd/disko.nix` and `modules/hosts/thinkpad/disko.nix`, each selected by its host. No generic storage module remains. Racknerd proposes BIOS GPT, first 1 MiB EF02, 2 GiB FAT `/boot`, remaining Btrfs `nix`/`persist`, and no swap. Its separately reviewed no-serial exception now selects the observed canonical `/dev/disk/by-path/pci-0000:00:04.0`, not raw `/dev/vda`. The operator approved complete replacement of that sole 100 GiB disk, with fresh identities and tested encrypted recovery. PCI paths name topology, not unique media; final VM/slot/capacity/current-use checks remain mandatory. Bastion's NVMe was freshly verified; ThinkPad's historical identifier still needs fresh review. EFI NVRAM writes are an explicit proposed policy on the UEFI hosts, not fresh boot acceptance.
...
Both models are upstream-supported ([pinned research](../../nix-research/references/research.md#fresh-install-storage-and-zfs-mounting--2026-09-12)):
...
**Superseded 2026-09-13:** the operator will administer the ThinkPad reinstall from their already-prepared MacBook, not Bastion. Both servers' candidates are now editor-free minimal secure baselines. Bastion loses the native agents capability, its public user-tmpfiles links, Pi/provider dependencies, Git/gh/tmux/devenv additions and whole-home persistence. Racknerd loses its editors capability; shared server policy keeps default nano disabled. Herdr was never added to either server. ThinkPad's desktop/HM agent configuration remains unchanged.

The previously accepted Bastion generation still has its private OS-backed `marcos` home and native tools until the operator deploys. Back up or privately migrate anything wanted before the [boot-only transition](../../deploy/references/operations.md#minimal-server-transition--2026-09-13). Removing the bind does **not** erase `/persist/home/marcos`, revoke any credentials stored there or remove old Nix generations. No forced live unmount, recursive home deletion, credential transfer or NAS change is part of this task.

MacBook preparation is operator-reported, not newly audited. Before a separately authorized ThinkPad wipe, keep the repository/lock, installer route, both reviewed administrator private keys, operator age identity, per-server signers and recovery independently usable outside that wipe. The repository's native validation/devenv outputs remain `x86_64-linux`; a Darwin deployment controller needs its prepared Linux build route and compatible pinned deploy-rs CLI, or uses a separate Linux checkout over SSH. Do not make either minimal server a replacement workspace or broaden Nix trust. ThinkPad remains unready/local-only; no new reinstall or activation authorization is implied.
...
2. **Recover:** record the OS no-backup choice distinctly from required NAS backup/restore review. Establish independent admin/SSH/SOPS recovery; Bastion now has its distinct recovered identity, shared-password membership and actual early-delivery rehearsal; installed two-boot acceptance is separately recorded in current status. New/replaced identities and ciphertext recipient changes need their own approval and private handling. Preserve or deliberately replace host identities; never copy another machine's keys.
3. **Implement and validate locally:** amend the storage ADR/runbook and capability/tests together; retain independent track/readiness inventories. Run native formatting and the canonical full gate for the coherent candidate, then required real readiness/build checks. Inspect generated boot/initrd/storage outputs without executing them. Only reviewed servers expose scripts; inspect the exact built artifact before any execution.
...
7. **Continue fleet:** only after Bastion acceptance, scope and complete Racknerd's provider-console/no-by-id/BIOS path. Reinstall ThinkPad **last**, after both servers are accepted and the operator's independent MacBook/controller and recovery prerequisites above are satisfied. Wiping the machine running the agent removes its local checkout and recovery generations.

## Validation evidence

Historical implementation baseline `a819153` passed all 23 checks. Subsequent final installation candidates passed their own full gates and real builds; both servers now have two-boot acceptance. The later minimal-server candidate passed 22 checks and both real server builds, but remains unactivated. The old Bastion workspace requirement is superseded by the MacBook/controller decision. [Validation records](../../validate/references/validation.md) distinguish this from older failed/interrupted runs and from the separately authorized real preflight operations. None of these checks is installed boot or administration-workspace acceptance.
