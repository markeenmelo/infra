...
5. Keep the service, its persisted state/ownership, host-specific policy, scripts, checks and developer/output integration together or adjacent by concern. Do not put unrelated configuration into base/common/misc or hardware facts. Use the lower evaluation's `pkgs` and `lib` for ordinary packages; assemble developer tools in native `devenv.nix` using its locked unstable `pkgs`, never host package sets; keep checks feature-owned, with Nix definitions under `modules/`, executable sources/tests under `scripts/<concern>/` and static data under `assets/<concern>/`.
6. For a genuine stable/unstable API difference, inspect pinned options/source on **both** tracks. Prefer one localized option-availability branch (see logging). Explain when to remove it and record evidence in `.agents/skills/nix-research/references/research.md`. Never solve compatibility by globally importing both package sets. A package exception needs a separate narrow, reviewed design.
...
8. Stage reviewed new files after the ciphertext guard, then complete [manual validation](../validate/SKILL.md). Build affected commissioned hosts separately. Review persistence migration, runtime secrets, required mounts and recovery before any later activation.
...
From repository root, enter `devenv shell` and follow [validate](../validate/SKILL.md) for the ordered manual checks; no repository tasks remain. For a commissioned consumer: `devenv shell ready HOST`, `devenv shell build HOST`. Bootstrap and CLI details: [development](../devenv/references/development.md).
...
Wrong module class, an unconsumed HM capability, outer/inner `config` confusion, lower-module files accidentally discovered at the top level (only `/_` helpers are excluded), recursive config-dependent imports, duplicate anonymous deferred imports, stale upstream options and unrelated package-channel injection. Inspect the module graph and pinned API; do not disable checks to hide them.
