...
Wrong module class, an unconsumed HM capability, outer/inner `config` confusion, lower-module files accidentally discovered at the top level or modules hidden by `/_` exclusions/symlinks, recursive config-dependent imports, duplicate anonymous deferred imports, stale upstream options and unrelated package-channel injection. Inspect the module graph and pinned API; do not disable checks to hide them.
