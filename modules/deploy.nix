...
let
  ...
  groupNames = [ "servers" "workstations" ];
  orderedHosts = group:
    lib.sort
      (a: b: if hosts.${a}.deployment.order == hosts.${b}.deployment.order then a < b else hosts.${a}.deployment.order < hosts.${b}.deployment.order)
      (lib.attrNames (lib.filterAttrs (_: host: host.deployment.group == group) hosts));
  mkNode = name: host: {
    inherit (host.deployment) hostname sshUser remoteBuild activationTimeout confirmTimeout;
    ...
    sshOpts = [
      "-p" (toString host.deployment.sshPort)
      "-o" "BatchMode=yes"
    ...
    profiles.system = {
      user = "root";
      path = (deployLib config.flake.fleetConfigurations.${name}.pkgs).activate.nixos config.flake.fleetConfigurations.${name};
    };
  };
  deploymentReport = lib.mapAttrs (
    track: fixture:
    let
      deploymentFor = sshUser:
        let
        ...
        in fixture.extendModules { modules = [ host.module ]; };
      deploy = deploymentFor "deploy";
      cfg = deploy.config;
      rejected = system:
        !(builtins.tryEval system.config.system.build.toplevel.drvPath).success;
    ...
    assert lib.assertMsg (
      ...
      && rejected (unsafe { users.users.deploy.hashedPassword = lib.mkForce ""; })
      && rejected (unsafe { users.users.deploy.openssh.authorizedKeys.keys = lib.mkForce [ ]; })
      && rejected (unsafe { nix.settings.trusted-users = lib.mkForce [ "root" ]; })
      && rejected (unsafe { security.sudo.extraRules = lib.mkForce [ ]; })
...
in
{
        ...
        config.module = { lib, ... }: {
          ...
          config = lib.mkIf deployment.enable {
            fleet.bootstrap.missing =
              ...
              ++ lib.optional (deployment.hostname != null && lib.hasSuffix ".invalid" deployment.hostname)
  ...
  config = {
    flake.modules.nixos.deploy = { config, lib, pkgs, ... }:
      let
        ...
        sudoRule = {
          users = [ "deploy" ];
          runAs = "root";
          commands = map (command: { inherit command; options = [ "NOPASSWD" ]; }) commands;
        };
      in {
        key = "fleet.deploy";
        imports = [ nixosModules.access nixosModules.ssh ];
        config = {
          ...
          assertions = [ {
            assertion =
              ...
              && lib.any (rule:
                rule.users == [ "deploy" ] && rule.runAs == "root"
    ...
    fleet.hosts = lib.mkMerge [
      (lib.genAttrs [ "racknerd" "bastion" "thinkpad" ] (_: { capabilities = [ "deploy" ]; }))
      {
        racknerd = {
          deployment = {
            enable = true;
    ...
    fleet.validation.hostChecks.deployment = { name, host, system }:
      ...
      assert lib.assertMsg (
        ...
        && host.deployment.sshUser == "deploy"
        && host.deployment.remoteBuild
        && config.flake.deploymentGroups.servers == [ "racknerd" "bastion" ]
    ...
    flake = {
      deploy.nodes = lib.mapAttrs mkNode (lib.filterAttrs (_: host: host.ready && host.deployment.enable) hosts);
      deploymentGroups = lib.genAttrs groupNames orderedHosts;
      deploymentPlan = lib.mapAttrs (_: host: host.deployment // {
        inherit (host) ready;
        profileUser = "root";
    ...
    perSystem = { pkgs, ... }: {
      ...
      checks = (deployLib pkgs).deployChecks config.flake.deploy
        // lib.concatMapAttrs (track: fixture:
          let smoke = (deployLib fixture.pkgs).deployChecks {
            nodes.fixture = {
              ...
              profiles.smoke = {
                user = "root";
                path = (deployLib fixture.pkgs).activate.custom
                  (fixture.pkgs.runCommand "smoke-payload" { } ''mkdir -p "$out"'') ":";
              };
            };
          };
          in lib.mapAttrs' (name: value: lib.nameValuePair "${track}-${name}-smoke" value) smoke
        ) fixtures;
    };
  };
}
